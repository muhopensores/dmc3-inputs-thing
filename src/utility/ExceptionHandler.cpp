...
LONG WINAPI reframework::global_exception_handler(struct _EXCEPTION_POINTERS* ei) {
    ...
    std::string crash_msg = fmt::format("Exception code {:x} occured @ ", ei->ExceptionRecord->ExceptionCode);
    ...
    if (module_within) {
        ...
        if (module_path) {
            ...
            crash_msg += fmt::format("Module: {:x} {}", (uintptr_t)*module_within, *module_path);
        } else {
            spdlog::error("Module: Unknown");
            crash_msg += fmt::format("Module: Unknown");
        }
    } else {
        spdlog::error("Module: Unknown");
        crash_msg += fmt::format("Module: Unknown");
    ...
    if (dbghelp) {
        ...
        const auto final_path = real_mod_dir + "crash.dmp";
        ...
        CloseHandle(f);
        crash_msg += " minidump has been written into crash.dmp file inside game directory.";
        MessageBoxA(NULL, crash_msg.c_str(), "Exception occured", MB_ICONEXCLAMATION);
    } else {
